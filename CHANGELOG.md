...
- **Ten new procedural characters** — Sable, Ashcroft, Bane, Bramble, Thorne, Quill, Rowan, Cassius, Lark and Barnaby, authored as recipes in `portraitArt.ts` alongside the office cast. Adds a `robe` garment that falls to a hem instead of legs, and a long-beard option. Portraits on agent cards come from the same recipes, so cards and floor sprites match.
