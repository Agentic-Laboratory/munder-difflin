...
**The Wizarding School office theme — the first show theme with art of its own.**
Behind the existing `tvShowOffices` flag, the sixth theme card is now real. A castle great hall: a high table at the head of the room, four long house tables split by a carpeted aisle, a potions classroom and a common room with a hearth and a tea trolley. Unlike the Brooklyn-99 theme, which is a layout painted with office tiles, this one ships an original tile atlas and an original cast. The card was previously labelled "Harry Potter" with the id `hogwarts` and marked unbuilt; it is now "Wizarding School", and in this fork the houses and characters carry their real names.

**The clickable floor props follow the theme now, and three more tabs are reachable from it.**
Building the castle exposed that the props you can click were never part of the Phase-1 theme extraction: the task-board choreography stands, the ASK ME board, the coffee steam and the clock's hit area still carried the office's own tile coordinates, in Brooklyn-99 as much as in the new theme. In the castle the pin and take stands landed inside a house table, so every task-ledger move played a thought bubble, never walked, and only caught up 30 seconds later through the watchdog — with archived notes stranded off the board that whole time. The floor also gains three new click targets, on furniture the map already paints.

### Added
- **Three new clickable props per theme** — `memory`, `skills` and `workers`, which had no way in from the floor before. Declared as `ThemeConfig.props`: the office gets its binder shelf, the vending machine and the conference table; the castle its common-room bookcase, the potions shelf and the brewing tables; the precinct gets evidence lockers, a chalkboard and a duty-roster board, painted for the purpose. The scene draws no prop art at all — only a hover outline and a label over art the map supplies, which is the split that keeps them from rotting the way the older props did.
- **Nine more contract assertions**, each verified by mutation: board stands walkable *and* reachable by flood fill, every wall prop hanging on a wall, clock art present and matching its hit-area extent, coffee objects with art beneath them and reachable stands, and every clickable prop on real art, opening a real `CCTab`, approachable, and never covering a walkable tile (which would make it swallow the click that selects an agent standing there). Two source guards keep the scene honest: no hardcoded tile position, and a clock hit area derived from the theme.
- **`wizardschool-castle.png`** — an original 256x112 tile atlas (98 tiles: flagstone, ashlar walls, arched and stained windows, oak doors, four house banners, torches, candelabra, hearth, refectory tables, benches, cauldrons and props), generated by `tools/wizardschool-art/gen-art.cjs` with a zero-dependency PNG encoder. Appended at `firstgid` 2449. No third-party licence — see `assets/tilesets/WIZARDSCHOOL-ART.md`.
- **A self-writing spellbook replaces the desk monitor.** Closed on the table, open and glowing while its reader is seated. Its page area is drawn to line up with the rect `DeskScreen` hardcodes for its scrolling-text animation, so the effect reads as writing appearing on the page.
- **Ten new procedural characters** — Dumbledore, McGonagall, Snape, Hagrid, Harry, Hermione, Ron, Draco, Luna and Neville, authored as recipes in `portraitArt.ts` alongside the office cast under the generic ids `sable`, `ashcroft`, `bane`, `bramble`, `thorne`, `quill`, `rowan`, `cassius`, `lark` and `barnaby`. Adds a `robe` garment that falls to a hem instead of legs, and a long-beard option. Portraits on agent cards come from the same recipes, so cards and floor sprites match.
- **`test/theme-contract.test.cjs`** — asserts, for every registered theme, that each name the `ThemeConfig` references exists in the map's spawn points, that the five tile layers and the `boardroom` zone are present, that every errand stand is walkable, that the seat marker is painted at the exact offset `OfficeFloor` looks for, and that no gid falls outside a declared atlas. These are all failures that were previously silent.
- **`tools/gen-wizardschool-map.cjs`** — authors the map and refuses to write it if anything is unreachable from the entrance or a seat marker is misplaced.

### Changed
- **Theme id `hogwarts` is now `wizardschool`** across the four duplicated config unions, the voice-settable allowlist and the picker. A saved config naming the old id is mapped to the new one by `resolveThemeId` in `themeLoader`, so anyone who selected the card while it was unbuilt keeps their choice instead of silently reverting to the office.
- **The Add-Agent character picker now offers only the active theme's roster** (`castForTheme`), while character *resolution* still accepts the full cast (`ALL_CAST`) so an agent hired under one theme survives a switch. Without the split, wizarding characters would appear in every user's picker with the flag off.
- `tools/mapgen/render_map.py` knows about the castle atlas, so the offline previewer can rasterize the new map.
- **`AnchorConfig` carries the geometry the scene used to hardcode** — `boardStands`, `askMe`, `clockSize` and `boardsPad` — and `CoffeeConfig` gains `machineTile`, because `machineStand` names the tile an agent stands on (the office's is `(26,20)`) while the steam was drawn at the machine top `(26,17)`, which no field described. `PaletteConfig.propStyle` moves the prop colors out of the scene: same geometry, parchment on oak for the castle where the office has paper on cork. The office's values are lifted verbatim, so its floor is pixel-identical.
- **Both map generators validate their prop anchors before writing** and paint the clock tile the quit prop needs (it draws no art of its own, so the anchor was an invisible click target). `gen-b99-map.cjs` also paints the break-room counter its coffee economy was missing — the mug rack, basin and steam had all been drawing over bare floor.
- **The castle cast and houses now carry their real names in this fork** — the ten display names are the Harry Potter cast and the four houses are Gryffindor, Slytherin, Ravenclaw and Hufflepuff, replacing the invented Emberwing / Nightthistle / Skyquill / Goldbriar set. Only *display* names changed: the internal ids are persisted in live roster data and stay generic, so no user's saved agent is disturbed. Neville moves to Gryffindor colours to match canon, which leaves the Hufflepuff scarf unworn while its banner and its stretch of house tables stay. The art is unaffected and remains original — `WIZARDSCHOOL-ART.md` now says plainly that the pixels are licence-clean and the names are borrowed, which is a local choice rather than something to carry upstream.
- **The great hall is lit like candlelight instead of a crypt.** Every masonry ramp in `gen-art.cjs` — flagstone, ashlar, the east-wing slate, the ironwork — was a cool blue-grey, so the banners, candelabra, carpet and oak tables were all fighting the walls instead of sitting in them; they are now a warm honey sandstone. The night sky, window glass and ambient glow stay deliberately cool, because that contrast is what sells the candles. The theme's canvas clear colour follows (`0x120f1c` → `0x1f1711`), as does the ASK ME frame, which was carrying much of the goth read on its own.

### Fixed
- **A themed floor's orchestrator reverted to Michael on every relaunch.** Every other agent carries its character in `roster.json`, but god's PTY never survives a restart, so `useHive` drops its roster entry and rebuilds the agent from scratch each launch — and that rebuild had `'Michael'` and `'michael'` written into it. The castle therefore booted with the office boss at the high table, under his own sprite, every single time. A theme's orchestrator is now declared in one place (`ROSTERS[theme].god`, read through the new `godForTheme`) and flows to god's card, its terminal tab and its `/remote-control` session; the boot loader and the schedules summary read it too, rather than naming Michael in visible copy. The theme is read from the store mirror, not raw config, because `App` resolves legacy ids and gates the feature on `tvShowOffices` before mirroring — so the raw value can name a theme the floor is not actually rendering. Four new contract assertions cover it, including a source guard that fails on a hardcoded name in god's spawn block.
- **The board-move watchdog was a free-running clock, so the note-carrying theatre only ever played in the first half-minute of a session.** It counted from scene start and reset only when it fired, so on any floor open longer than its timeout the first frame after a move began tripped it: the note vanished from the actor's hand and the boards snapped to the ledger while they were still walking there. It now counts only while a move is actually in flight, and the budget went from 30s to 45s because the longest round trip a theme asks for — the castle's farthest seat to the common-room boards and back — is about 28s at `Character.ts`'s walk speed. A new assertion measures that worst case per theme by flood fill and fails if it does not fit with 20% to spare; at the old 30s even the office (25.6s) would not have passed it.
