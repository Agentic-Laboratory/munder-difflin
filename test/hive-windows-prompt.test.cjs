...
// UPSTREAM TEST DROPPED ON PURPOSE — not a coverage gap.
// v0.4.4 also fixed the mixed-separator path in `HiveManager.drainForStop()` and
// pinned it here. This fork RETIRED that method in 98761ea5: it had been dead
// since 6d1a73b, three weeks with zero callers (the Stop-hook drain runs through
// the hook bridge, not through it). Keeping the test would mean resurrecting dead
// code purely to satisfy an assertion, so the test goes instead. Every other
// native-separator path the fix touches is still pinned by the
// 'protocol paths use native separators' test above.
