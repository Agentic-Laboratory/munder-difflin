...
export class HiveManager {
  ...
  private injectedPrompt(
    ...
    const integrationLine = 'INTEGRATION / GIT SAFETY — read before any git write: (1) NEVER run git checkout, git merge, or git rebase inside a repo that has a dev server running against it. This app is launched with `npm run dev` and vite watches the working tree, so a conflicted merge writes <<<<<<< markers into files on disk and breaks the RUNNING app mid-session, and a branch switch silently serves a different branch’s code to the live window. Integrate in a throwaway worktree instead: `git worktree add <tmp> <branch>`, merge and run the gates there, then `git worktree remove <tmp>` and fast-forward the real branch. (2) NEVER run a bare `git push`. Always name the remote explicitly, because in some clones `origin` is an upstream PARENT repo that must only ever receive a PR — pushing to it by accident is not something you can take back.';
